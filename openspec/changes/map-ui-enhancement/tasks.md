...
- [x] 5.1 修改 `static/js/map.js` 中的地圖邊框色（`stroke`），根據目前主題動態調整：亮色模式 `#fff`、暗色模式 `#555`。驗收：暗色模式下地圖區塊邊框清晰可見
- [x] 5.2 修改 SVG 內嵌標題文字色彩，根據目前主題動態調整。驗收：暗色模式下標題文字清晰可讀

## 6. 測試與驗證

- [x] 6.1 重啟 Docker container 並在瀏覽器中驗證：縣市地圖色彩改為人口數指標、標題位於地圖內左上角、暗色模式切換正常。驗收：三項功能均正常運作
